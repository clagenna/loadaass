...
public class P03TableviewEditable extends Application {
  ...
  public void start(Stage stage) {
    ...
    firstNameCol.setOnEditCommit(ev -> cambiaFirst(ev));
    ...
    lastNameCol.setOnEditCommit(ev -> cambiaLast(ev));
    ...
    emailCol.setOnEditCommit(ev -> cambiaEmail(ev));
  ...
  private Object cambiaFirst(CellEditEvent<P02Person, String> ev) {
    String val = ev.getNewValue();
    TableView<P02Person> tbl = ev.getTableView();
    int k = ev.getTablePosition().getRow();
    P02Person rec = tbl.getItems().get(k);
    rec.setFirstName(val);
    return null;
  }

  private Object cambiaLast(CellEditEvent<P02Person, String> ev) {
    String val = ev.getNewValue();
    TableView<P02Person> tbl = ev.getTableView();
    int k = ev.getTablePosition().getRow();
    P02Person rec = tbl.getItems().get(k);
  ...
  private Object cambiaEmail(CellEditEvent<P02Person, String> ev) {
    String val = ev.getNewValue();
    TableView<P02Person> tbl = ev.getTableView();
    int k = ev.getTablePosition().getRow();
    P02Person rec = tbl.getItems().get(k);
